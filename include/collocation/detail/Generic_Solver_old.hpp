...
void Generic_Solver<Method, Singular>::modify_mesh(Scalar tol) {
  ...
  for (Index idx = 0; idx < m_method.cols() - 1; ++idx) {
    if (m_method.residuals(idx) > tol) {// Add nodes
      if (m_method.residuals(idx) >= m_tol_factor * tol) {// Add two nodes
      ...
      if (intervals > 2) {
        ...
        constexpr Scalar order_inv = static_cast<Scalar>(1) / order;
        Map<const Array<Scalar, Dynamic, 1>> x_sub(m_method.x.data() + idx, intervals);
        Array<Scalar, Dynamic, 1> rms_new = m_method.residuals.segment(idx, intervals);
        Array<Scalar, Dynamic, 1> h_new = m_method.h.segment(idx, intervals);
        Index i_min;
        // Scalar min = rms_new.segment(1, intervals - 2).minCoeff(&i_min);
        rms_new.segment(1, intervals - 2).minCoeff(&i_min);
        i_min += 1;
        Scalar hsum = h_new.template segment<3>(i_min - 1).sum();
        Scalar rim1_pinv = pow(rms_new(i_min - 1), order_inv);
        Scalar rip1_pinv = pow(rms_new(i_min + 1), order_inv);
        ...
        Scalar r_new = rms_new(i_min - 1) * rms_new(i_min + 1) * pow(hsum / (h_new(i_min + 1) * rim1_pinv + h_new(i_min - 1) * rip1_pinv), order);
        rms_new(i_min - 1) = r_new;
        rms_new(i_min + 1) = r_new;
        rms_new.segment(i_min, intervals - i_min - 1) = rms_new.tail(intervals - i_min - 1);
        rms_new.conservativeResize(intervals - 1);
        while (r_new < half<Scalar>() * tol) {
          ...
          m_nodes_removed += 1;
          if (intervals > 2) {
            // min = rms_new.segment(1, intervals - 2).minCoeff(&i_min);
            rms_new.segment(1, intervals - 2).minCoeff(&i_min);
            i_min += 1;
            hsum = h_new.template segment<3>(i_min - 1).sum();
            rim1_pinv = pow(rms_new(i_min - 1), order_inv);
            rip1_pinv = pow(rms_new(i_min + 1), order_inv);
            ...
            r_new = rms_new(i_min - 1) * rms_new(i_min + 1) * pow(hsum / (h_new(i_min + 1) * rim1_pinv + h_new(i_min - 1) * rip1_pinv), order);
            rms_new(i_min - 1) = r_new;
            rms_new(i_min + 1) = r_new;
            rms_new.segment(i_min, intervals - i_min - 1) = rms_new.tail(intervals - i_min - 1);
            rms_new.conservativeResize(intervals - 1);
        ...
        for (Index i = 0; i < h_new.size(); ++i) {
          ...
          Scalar w = (x_new(idx_new + i) - m_method.x(idx + ii)) / m_method.h(ii);
          y_new.col(idx_new + i) = m_method.S(ii, w);
        }
        idx_new += h_new.size();
        ...
        // y_new.block(0, idx_new, n, intervals) = m_method.y.block(0, idx + 1, n, intervals);

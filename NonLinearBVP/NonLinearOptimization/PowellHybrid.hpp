...
#include "NonLinearBVP/NonLinearOptimization/dogleg.hpp"
...
public:
  ...
  struct Parameters {
    Parameters() : factor{100}, max_nfev{1000}, max_njev{4}, a_tol{Eigen::numext::sqrt(Eigen::NumTraits<Scalar>::epsilon())}, r_tol{Eigen::numext::sqrt(Eigen::NumTraits<Scalar>::epsilon())}, bc_tol{Eigen::numext::sqrt(Eigen::NumTraits<Scalar>::epsilon())}, epsfcn{0} {}
    Scalar factor;
    Index max_nfev;
    Index max_njev;
    Scalar a_tol;
...
PowellHybridSolverSpace::Status PowellHybrid<Method, Singular>::solveInit() {
  ...
  if (n <= 0 || parameters.a_tol < 0 || parameters.r_tol < 0 || parameters.bc_tol < 0 || parameters.max_nfev <= 0 || parameters.max_njev <= 0 || parameters.factor <= 0) {
...
PowellHybridSolverSpace::Status PowellHybrid<Method, Singular>::solveOneStep() {
  ...
  while (true) {
    ...
    if (ratio >= ten_thousandth<Scalar>()) {
      m_method.step(wa1); // Step the solution
      if constexpr (Singular) { // Apply singular transformation to the origin if needed
        m_method.apply_singular(this->m_B);
      }
      m_method.calculate(); // Calculate the internal points and new residues
      if (m_method.residues_converged(parameters.a_tol, parameters.r_tol) && (m_method.bc_residues.abs() <= parameters.bc_tol).all()) { // test for convergence
      ...
      fnorm = fnorm1;
      wa2.noalias() = wa4 - wa3;
      marwil(m_method.jacobian, wa1, wa2, diag); // Approximate the new Jacobian
      // m_method.construct_global_jacobian();
      m_solver.factorize(m_method.jacobian);
      if (m_solver.info() != Success) { // Calculate the exact Jacobian before giving up
        m_method.construct_global_jacobian();
        m_solver.factorize(m_method.jacobian);
        ++njev;
        if (m_solver.info() != Success) { // Jacobian still singular, give up now.
          return PowellHybridSolverSpace::SingularJacobian;
        }
      }
      ++iter;

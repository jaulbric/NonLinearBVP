...
public:
  ...
  Array<Scalar, RowsAtCompileTime, 1> operator()(Scalar x) const {
    ...
    if (x == m_x(last)) {
      return m_y.col(m_cols - 1);
    }
    else if (x < m_x(0) || x > m_x(last)) {
  ...
  // Array<Scalar, RowsAtCompileTime, 1> operator()(Scalar x) const {
  Array<Scalar, RowsAtCompileTime, 1> unchecked(Scalar x) const {
  ...
  template <typename Derived>
  Array<Scalar, RowsAtCompileTime, Derived::SizeAtCompileTime> unchecked(const ArrayBase<Derived>& x) const {
    assert(x.rows() == 1 || x.cols() == 1);
    Array<Scalar, RowsAtCompileTime, Derived::SizeAtCompileTime> ret(m_rows, x.size());
    for (Index idx = 0; idx < x.size(); ++idx) {
      ret.col(idx) = unchecked(x(idx));
    }
    return ret;
  }
